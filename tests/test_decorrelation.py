...
from dstretch import DecorrelationStretch, ColorspaceManager
...
class TestDecorrelationStretch:
    ...
    def test_process_rgb_identity(self):
        ...
        difference = np.mean(np.abs(result.processed_image.astype(float) - test_image.astype(float)))
    ...
    def test_process_different_colorspaces(self):
        ...
        difference = np.mean(np.abs(rgb_result.astype(float) - yds_result.astype(float)))
    ...
    def test_scale_effect(self):
        ...
        diff_low = np.mean(np.abs(result_low.processed_image.astype(float) - test_image.astype(float)))
        diff_high = np.mean(np.abs(result_high.processed_image.astype(float) - test_image.astype(float)))
    ...
    def test_reset_functionality(self):
        ...
        # Process image
        result = dstretch.process(test_image, "YDS", scale=25.0)
...
@pytest.mark.skipif(not Path("test_image.jpg").exists(), reason="No test image available")
def test_real_image_processing():
    """Integration test with a real image file."""
    result = process_image("test_image.jpg", colorspace="YDS", scale=15.0)
