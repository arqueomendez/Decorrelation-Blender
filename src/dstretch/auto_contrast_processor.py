...
class AutoContrastProcessor:
    ...
    def process(self, image: np.ndarray) -> np.ndarray:
        ...
        # Log processing info
        logger.info(f"Processing auto contrast - Shape: {image.shape}, "
    ...
    def _stretch_independent_channels(self, image: np.ndarray) -> np.ndarray:
        ...
        for channel in range(3):
            ...
            result[:, :, channel] = self._apply_linear_stretch(channel_data, min_val, max_val)
    ...
    def _find_histogram_cutoffs(self, histogram: np.ndarray, total_pixels: int) -> Tuple[float, float]:
        """
    ...
    def _apply_linear_stretch(self, channel: np.ndarray, min_val: float, max_val: float) -> np.ndarray:
        """
...
def main():
    ...
    test_image = np.random.randint(80, 120, (100, 100, 3), dtype=np.uint8)  # Low contrast
    
    # Test different contrast modes
    processor_preserve = AutoContrastProcessor(clip_percentage=0.1, preserve_colors=True)
    processor_independent = AutoContrastProcessor(clip_percentage=0.1, preserve_colors=False)
    ...
    print(f"Enhanced range (preserve): {np.min(enhanced_preserve)} - {np.max(enhanced_preserve)}")
    print(f"Enhanced range (independent): {np.min(enhanced_independent)} - {np.max(enhanced_independent)}")
